...
import { BOB_Token } from 'constants/tokens'
...
import { ReactComponent as Switch } from '../../assets/svg/arrowsnew.svg'
...
import NetworkSelectorWhiteModal from '../../components/NetworkSelectorWhiteModal'
...
import { BOB_SWAP, CHAIN_SWAP_NAMES, LOGO, SUSHI_SWAP, UNKNOWN_LOGO } from '../../constants/addresses'
...
const Logo = styled.img<{ darkMode: boolean }>`
  ...
  `}
`
const RouteFlyout = styled.span<{ flyoutAlignment?: FlyoutAlignment }>`
  min-width: 220px;
  max-height: 350px;
  overflow: auto;
...
export default function Swap({ history }: RouteComponentProps) {
  ...
  const [isRouteFlyoutVisible, setIsRouteFlyoutVisible] = useState(false)
  ...
  // meh
  const {
    v3Trade: { state: v3TradeStateKiba },
    bestTrade: tradeKiba,
    allowedSlippage: allowedSlippageKiba,
    currencyBalances: currencyBalancesKiba,
    parsedAmount: parsedAmountKiba,
    currencies: currenciesKiba,
    inputError: swapInputErrorKiba,
  } = useDerivedSwapInfo(SUSHI_SWAP, toggledVersion)

  const tradeMapInit: TradeMap = {
    [SUSHI_SWAP]: {
      trade: tradeKiba,
      v3TradeState: v3TradeStateKiba,
      allowedSlippage: allowedSlippageKiba,
      currencyBalances: currencyBalancesKiba,
      parsedAmount: parsedAmountKiba,
      currencies: currenciesKiba,
      swapInputError: swapInputErrorKiba,
      name: SUSHI_SWAP,
    },
  }
  ...
  useEffect(() => {
    if (!userHasSelected.current && sortedTrades.length > 0) {
      const isBobInInput = currencies[Field.INPUT]?.equals(BOB_Token) ?? false
      const isBobInOutput = currencies[Field.OUTPUT]?.equals(BOB_Token) ?? false
      const filteredSwaps = sortedTrades.filter((swap) => {
        if (isBobInInput || isBobInOutput) {
          return swap.name === BOB_SWAP
        }
        return swap.name !== BOB_SWAP
      })
      if (filteredSwaps.length === 1 && filteredSwaps[0].name === BOB_SWAP) {
        setSelectedSwap(BOB_SWAP)
      } else if (sortedTrades.length > 0) {
        setSelectedSwap(sortedTrades[0].name)
      }
    }
  }, [sortedTrades, setSelectedSwap, userHasSelected, currencies])
  ...
  const [validRoutesCount, setValidRoutesCount] = useState<number>(0)
  useEffect(() => {
    const count = sortedTrades.filter(({ amountOut }) => parseFloat(amountOut) > 0).length
    setValidRoutesCount(count)
  }, [sortedTrades])
  ...
  return (
    <>
      ...
      />
      <NetworkAlert />
      <HeaderText>BobSwap </HeaderText>
      <SubheaderText>Powered by KibaSwap</SubheaderText>
      <AppBody>
        <SwapHeader allowedSlippage={allowedSlippage} />
        <Wrapper
          id="swap-page"
          isMobile={isMobile}
          style={{ background: theme.bg0, borderRadius: 24, paddingTop: 0, padding: isMobile ? 20 : 32 }}
        >
          <ConfirmSwapModal
            isOpen={showConfirm}
            trade={trade}
          ...
          <AutoColumn gap={'sm'}>
            <div style={{ display: 'relative', marginBottom: '20px' }}>
              <CurrencyInputPanel
                label={
                ...
                customNode={
                  <>
                    {validRoutesCount > 2 && (
                      <ButtonDropdown
                        onClick={() => {
                          setIsRouteFlyoutVisible(!isRouteFlyoutVisible)
                        }}
                      >
                        Multiple LP Paths{' '}
                      </ButtonDropdown>
                    )}
                    {isRouteFlyoutVisible && (
                      <RouteFlyout>
                        {sortedTrades
                          .filter((swap) => {
                            const isBobInInput = currencies[Field.INPUT]?.equals(BOB_Token) ?? false
                      ...
                      <AutoRow justify="space-between" style={{ flexWrap: 'nowrap' }}>
                        ...
                        {approvalState === ApprovalState.PENDING ? (
                          ...
                          <MouseoverTooltip
                            text={
                              <Trans>
                                You must give the BOBSWAP smart contracts permission to use your{' '}
