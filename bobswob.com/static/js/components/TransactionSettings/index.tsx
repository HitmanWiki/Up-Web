...
export default function TransactionSettings({ placeholderSlippage }: TransactionSettingsProps) {
  ...
  function parseSlippageInput(value: string) {
    ...
    if (value.length === 0) {
      ...
      if (!Number.isInteger(parsed) || parsed < 0 || parsed > 90000) {
        ...
        if (parsed < 110) {
          setSlippageInput('too low')
          setTimeout(() => {
            setSlippageInput('1.1')
            setUserSlippageTolerance(new Percent(110, 10_000))
          }, 1000)
        } else {
  ...
  const tooLow = userSlippageTolerance !== 'auto' && userSlippageTolerance.lessThan(new Percent(100, 10_000))
