...
export default function WalletModal({
  ...
  const WalletDropdown = forwardRef((props: BoxProps, ref: ForwardedRef<HTMLElement>) => {
    const isMobileW = useIsMobile()
    return <Box ref={ref} className={isMobileW ? styles.mobileNavDropdownTop : styles.WalDropdown} {...props} />
  })
        ...
        if (nameParam === 'BOB PRO') {
          const bobInterval = setInterval(() => {
            if (connector && connector.walletConnectProvider && first) {
              ...
              //   target: '_blank',
              //   href: android ? `https://bob.app/wc?uri=${uri}` : `bob://bob.app/wc?uri=${uri}`,
              // }).click()
              window.location.href = android ? `https://bob.app/wc?uri=${uri}` : `bob://bob.app/wc?uri=${uri}`
              first = false
            }
          }, 1000)
          setTimeout(() => {
  ...
  function getModalContent() {
    ...
    return (
      <UpperSection>
        ...
        <ContentWrapper>
          <AutoColumn gap="16px">
            <LightCard style={{ width: isMobile ? '100%' : '300px' }}>
              <AutoRow style={{ flexWrap: 'wrap' }}>
                <TYPE.black fontSize={14} style={{ whiteSpace: 'pre-wrap', overflowWrap: 'break-word', width: '100%' }}>
                  <Trans>
                    By connecting your wallet, you agree to the{' '}
                    <ExternalLink href="https://bob.swap-terms-of-service">Terms of Service</ExternalLink> and{' '}
                    <ExternalLink href="https://bob.swap-disclaimer">Protocol Disclaimer</ExternalLink>.
                  </Trans>
                </TYPE.black>
              </AutoRow>
            </LightCard>
